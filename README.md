...
> 💡 **`gemini-3.5-flash` 로 고정해도 쿼터 문제는 해결되지 않습니다.** 기본값이 이미 3.5 Flash 로
> 해석되므로 동일하며, `429`(일일 쿼터 소진)도 3.5 Flash 에서 발생합니다. 쿼터에 여유가 필요하면
> 구세대라 무료 한도가 더 큰 **`gemini-2.5-flash`** 로 고정하는 것이 실효가 있습니다.
...
| `gemini-2.5-flash` | 2.5 Flash | 중간 (~250) | **`429` 가 자주 날 때** — 쿼터 여유 |
...
- **품질을 원하면** 그대로 두세요(= 최신 3.5 Flash). 나중 버전으로 자동 업그레이드를 막고 3.5 로
  고정하고 싶을 때만 `gemini-3.5-flash` 를 지정합니다.
- **`429` 가 자주 나면** `gemini-2.5-flash` 로 고정하세요. (`rerun-review` 는 호출을 1회 더 소모하므로
  입력을 확실히 보완한 뒤 사용하고, 실패 대처는 [트러블슈팅](#review-failed-라벨이-붙은-경우) 참고)
