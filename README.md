...
2. 저장소 **Settings → Secrets and variables → Actions → New repository secret**
   - Name: `GEMINI_API_KEY`
   - Value: 발급받은 키
3. (선택) 모델 변경 시 **Variables** 탭에서 `GEMINI_MODEL` 추가.
   - **기본값은 `gemini-flash-latest`** — 항상 최신 Flash 버전으로 자동 실행되어 검토 품질을
     확보합니다(현재는 **Gemini 3.5 Flash** 로 해석됨). 실제 사용된 버전은 검토 결과 **최상단의
     `모델 정보`** 줄에 표시됩니다.
   - 버전을 고정하려면 `gemini-3.5-flash`, `gemini-2.5-flash` 등을 지정하세요. 무료 티어에서
     일일 검토 가능 건수를 늘리려면 `gemini-2.5-flash-lite` 를 고려하세요.
...
| --- | --- | --- | --- |
| `gemini-flash-latest` (**기본값**) | 해석되는 버전 따름 | 해석되는 버전 따름 | 항상 최신 Flash 로 자동 실행(현재 → `gemini-3.5-flash`) |
| `gemini-2.5-flash` | ~10 | ~250 | 품질·쿼터 균형이 좋은 안정 버전 |
| `gemini-2.5-flash-lite` | ~15 | ~1,000 | 검토량이 많을 때. 품질은 다소 낮아질 수 있음 |
| `gemini-2.5-pro` | ~5 | ~100 | 가장 정밀하지만 무료 한도가 작음 |

**운영 팁**

- 일반적인 사내 검토량(하루 수~수십 건)이라면 기본값 `gemini-flash-latest` 무료 한도로 충분합니다.
...
\* 입력 5K + 출력 8K tokens 가정, 그라운딩 무료 구간 기준. 기본값 `gemini-flash-latest` 는 현재 최신
Flash(`gemini-3.5-flash`)로 해석되므로 검토 1건 ≈ $0.08 수준입니다. 비용을 낮추려면 `gemini-2.5-flash`
(약 1/4) 또는 `gemini-2.5-flash-lite`(약 1/20)로 고정하세요.
...
> 💡 비용이 부담되면 `GEMINI_MODEL` 을 `gemini-2.5-flash`(약 1/4) 로 고정하세요. 위 표의 유료 비용은
> 무료 한도 초과 시 참고치이며, 이 앱은 무료 티어에서 **$0** 로 동작합니다.
