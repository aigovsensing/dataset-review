...
- **소개 · 구조**
  - [🧩 한눈에 보기](#-한눈에-보기) · [🔄 데이터 흐름](#-데이터-흐름-data-flow) · [⚙️ 동작 흐름](#-동작-흐름-operation-flow) · [구성 요소](#구성-요소)
- **시작하기**
...
- **운영 · 참고**
  ...
  - [무료 Gemini API 안정 운영](#-무료-gemini-api-안정-운영-호출-최소화-설계)
    - [모델 선택과 무료 한도](#모델-선택과-무료-한도-gemini_model) · [무료 티어 사용 가능 모델](#무료-티어-사용-가능-모델) · [503 자동 복구](#503일시-과부하-대응--3단계-자동-복구) · [예상 API 비용](#-예상-ai-api-비용-검토-요청-횟수별)
...
flowchart TD
    ...
    B -->|"아니오"| X["건너뜀"]
    B -->|"예"| C{"작성자 멤버 또는 승인됨"}
    C -->|"아니오"| X2["건너뜀 · 쿼터 보호<br/>(멤버가 review-approved 로 승인)"]
    C -->|"예"| D{"검토 트리거 이벤트"}
    D -->|"아니오"| X3["건너뜀"]
