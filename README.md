...
| --- | --- | --- | --- | --- |
| `gemini-2.5-flash` (기본값) | $0.30 / 1M | $2.50 / 1M | **≈ $0.02** | +$0.035/건 (일 1,500건 무료 후) |
...
\* 입력 5K + 출력 8K tokens 가정, 그라운딩 무료 구간 기준. `gemini-3.5-flash` 는 기본값의 약 **4배**,
`gemini-2.5-flash-lite` 는 약 **1/5** 수준입니다.

#### ② 검토 요청 횟수별 예상 비용 (기본 모델 `gemini-2.5-flash`)

| 검토 요청 횟수 | 실제 Gemini 호출 횟수 | 무료 티어 비용 | 유료 티어 예상 비용 | 비고 |
| --- | --- | --- | --- | --- |
| 1건 | 1회 (드물게 2회) | **$0** | ≈ $0.02 | 그라운딩 일 1,500건 무료 내 |
| 10건 | ≈ 10회 | **$0** | ≈ $0.2 | |
| 100건 | ≈ 100회 | **$0** | ≈ $2 | |
| 1,000건 | ≈ 1,000회 | $0 (여러 날 분산 시) | ≈ $22 | 무료 일일 한도(RPD) 초과 가능 → 유료 필요 |
| 10,000건 | ≈ 10,000회 | 한도 초과 | ≈ $220 (+그라운딩 초과분) | 하루 1,500건 초과분은 그라운딩 $0.035/건 별도 |
