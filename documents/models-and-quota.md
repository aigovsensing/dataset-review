...
1. **정상 검토는 Gemini API 호출 1회.** 다만 오류 재시도, 모델·키 회전, 하이브리드 2패스,
   또는 모든 그라운딩 실패 뒤의 PLAIN 최후 폴백에서는 추가 호출이 발생할 수 있습니다.
...
- **무그라운딩 최후 폴백:** 모든 API 키에서 그라운딩이 쿼터·모델 가용성 문제로 실패하면,
  3.x 모델을 검색 도구 없이 다시 호출해 참고용 결과라도 남깁니다. 이 결과에는 웹 출처와 최신성
  보장이 없음을 상단 경고와 생성 각주에 명시하며, 인증·입력·코드 오류에는 적용하지 않습니다.
