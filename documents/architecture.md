...
- **논문 기반 분석**: 이슈에 논문 주소(예: `arxiv.org/abs/...`)를 입력하면 Gemini 가 Google
  ...
  문장을 출처와 함께 인용합니다. 논문 검토 결과에는 Abstract 기반의 **논문 초록**과 외부·자체 생성
  데이터셋별 **실험 사용 목적**(학습·검증·평가·벤치마크 등)도 근거 위치와 함께 출력합니다.
- **LLM 생성 데이터 검토**: 데이터가 LLM으로 생성되었다면 서비스명뿐 아니라 공식 자료에서 확인되는
  실제 모델명·버전을 밝히고, 제공사의 공식 이용약관과 모델 라이선스를 근거로 데이터셋 생성·상업적
  이용·재배포 조건 및 약관으로 해소되지 않는 법적 리스크까지 구분해 검토합니다.
